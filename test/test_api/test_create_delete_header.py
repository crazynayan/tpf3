...
class CreateDeleteHeader(TestAPI):
    ...
    def test_create(self):
        ...
        self.assertDictEqual(self.create_response, response.json())
    ...
    def test_delete_success(self):
        ...
        self.assertEqual(200, response.status_code)
        self.assertDictEqual(success_response, response.json())
    ...
    def test_get_all_and_get_by_name(self):
        ...
        response = self._create_test_data(TestAPI.NAME)
        get_by_name_response = [response.json()]
        get_all_response.append(response.json())
        response = self._create_test_data(f"{TestAPI.NAME} - 2")
        get_all_response.append(response.json())
        # Get All Headers
        response = self.get(f"/api/test_data")
        self.assertEqual(200, response.status_code)
        self.assertListEqual(get_all_response, response.json())
        # Get by name - All Types
        response = self.get(f"/api/test_data", params={NAME: TestAPI.NAME})
        self.assertEqual(200, response.status_code)
        self.assertListEqual(get_by_name_response, response.json())
